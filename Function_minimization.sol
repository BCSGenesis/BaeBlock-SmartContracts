...
// 변수선언 -----------------------------------------------------------------------
    ...
    struct Order {
        ...
        uint deliveryFee;   //거리에 따라 지정되는 배달비
...
// 결제 ------------------------------------------------------------------------------
    ...
    function storeAccept_Decline(uint _orderID, bool) public payable {
        ...
        if(true){
            ...
            uint refundPrice = searchOrder[orderID].foodPrice + searchOrder[orderID].deliveryFee + searchOrder[orderID].deliveryTip;                     
            payable (searchOrder[orderID].cWallet).transfer(refundPrice);                               //환불  //??????????컨트랙트가 고객에게 확인! transferfrom사용해야할텐데....?
        }
    }
